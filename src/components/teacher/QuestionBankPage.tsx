...
export default function QuestionBankPage() {
  ...
  if (isUnderDevelopment) {
    return (
      <div style={{ minHeight: '100vh', background: S.bg, color: S.text, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
        <div style={{ maxWidth: 560, width: '100%', background: S.card, border: `1px solid ${S.border}`, borderRadius: 16, padding: 28, textAlign: 'center' }}>
          <div style={{ fontSize: 42, marginBottom: 10 }}>🚧</div>
          <h1 style={{ fontSize: 24, margin: 0, marginBottom: 8 }}>Ngân hàng đề thi</h1>
          <p style={{ fontSize: 14, color: S.muted, margin: 0, marginBottom: 18 }}>
            Tính năng này đang phát triển và sẽ sớm được cập nhật.
          </p>
          <button
            onClick={() => router.push('/teacher')}
            style={{ padding: '9px 16px', borderRadius: 10, border: 'none', background: '#185FA5', color: '#fff', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}
          >
